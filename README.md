...
*   **Google Drive 远程备份**：支持连接 Google Drive，手动备份、云端恢复，以及本地数据变更后的自动备份。
*   **云端 JSON 查看**：支持直接查看当前 Google Drive 中保存的备份 JSON 内容。
...
完成授权并至少执行过一次备份后，你可以在设置页点击 `查看云端 JSON`，直接查看当前保存在 Google Drive 中的备份内容。当前展示的是格式化后的只读 JSON，便于检查远程备份是否成功。
